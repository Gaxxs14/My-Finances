...
namespace MyFinances.API.Controllers
{
    ...
    public class SyncController : ControllerBase
    {
        ...
        public async Task<IActionResult> ResetUserData()
        {
            ...
            var userAccounts = await _context.Accounts.Where(a => a.UserId == userId.Value).ToListAsync();
            var accountIds = userAccounts.Select(a => a.Id).ToList();

            var userTransactions = await _context.Transactions.Where(t => accountIds.Contains(t.AccountId)).ToListAsync();
            _context.Transactions.RemoveRange(userTransactions);
            _context.Accounts.RemoveRange(userAccounts);

            var userCredentials = await _context.Credentials.Where(c => c.UserId == userId.Value).ToListAsync();
            _context.Credentials.RemoveRange(userCredentials);

            await _context.SaveChangesAsync();
            return Ok(new { Message = "Todos los datos de tu cuenta en la nube han sido eliminados." });
        }
    }
}
